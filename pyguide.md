...
# Function Health Python Style Guide
...
Python is the main dynamic language used at Function Health. This style guide is a list
of *dos and don'ts* for Python programs and orginally forked from Google's style guide.
...
Run `Ruff` over your code using the [default_ruff_config.toml](https://github.com/Function-Health/actions/blob/main/python/ruff/v1/default_ruff_config.toml). Helful fu

Helpful commands to run Ruff:
```shell
...
```python
def do_PUT(self):  # ruff: noqa N802 invalid-function-name
  ...
```

Add the exact rule to disallow along with the human readable naming for others
to quickly understand what the disallow is doing.
...
```shell
ruff check path/to/code/ --select W
```

To get more information on a particular message, enter the rule in [Ruff search](https://docs.astral.sh/ruff/rules/).
For example [N802](https://docs.astral.sh/ruff/rules/invalid-function-name/).

For directories that should not be reviewed by Ruff you can disallow the path
or file within the `pyproject.toml` files. The use of this is discouraged.
...
Module-level constants are permitted and encouraged and should include the type
`Final`. For example:

```python
from typing import Final
_MAX_HOLY_HANDGRENADE_COUNT: Final = 3  # for an internal use constant or
SIR_LANCELOTS_FAVORITE_COLOR: Final = "blue" #  for a public API constant
```

Constants must be named using all caps with underscores. See [Naming](#s3.16-naming)
below.
...
the module level so that it can still be accessed by tests. For example `def _foo():`.
...
You should annotate Python code with type hints according to
[PEP-484](https://peps.python.org/pep-0484/), and type-check the code at build
time with [pyright](https://github.com/microsoft/pyright).

Type annotations can be in the source for local configuration but should be run
from Github Action.
...
the issue(s) currently preventing type annotation adoption in the `pyproject.toml`
file or in the code itself as appropriate.
...
- Long string module-level constants not containing whitespace that would be
  ...
  - Ruff disable comments. (e.g.: `# noqa: E501 line-too-long`)
...
comma is also used as a hint to our Ruff auto-formatter to direct it to
...
`pytest //foo/bar:foo_test -- --update_golden_files` from the `test`
directory.
"""
```
...
Including `Args`, `Returns`, and `Raises` should only be included when the function
includes one of these attributes. For example `Returns` can be removed if the
...
Attributes should be declared explicitly with their types alongside a docstring
...
class SampleClass:
    ...
    A property docstring also supports multiline comments.
    """
...
A `TODO` comment begins with the word `TODO` in all caps, a following colon, your username in parentheses, and
a link or Jira ticket refrence. A bug
...
```python
# TODO(yourusername): Investigate cpufreq optimizations.
# FHC-14
```
...
If the `TODO` do not require a bug to be added, add `NOBUG-1` to the end
of the `TODO` to supress the Ruff error.

```python
# TODO(yourusername): Refactor this function.
# NOBUG-1
```
...
some code. Do not be intimidated by modifying existing code: if working with such a
...
CapWorded. If the alias is used only in this module, it should be \_Private and
placed after the imports.
...
`pyright` has a disable option for specific errors (similar to lint) that follows
